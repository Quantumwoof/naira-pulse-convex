...
function DemoPreview() {
  ...
  const [status, setStatus] = useState("Local DEMO preview (Convex URL not set)");
  ...
  async function refresh() {
    ...
    setStatus(`Refreshed ${next.length} DEMO fixture pulses (no Convex yet)`);
    setBusy(false);
  }
...
function LiveDashboard() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setStatus(
        `Refreshed ${result.count} pairs via ${result.mode.toUpperCase()} · ${formatTime(result.capturedAt)} WAT`,
      );
    } catch (e) {
  ...
  return (
    <DashboardShell
      ...
      recent={recent ?? []}
      tip={tip}
      busy={busy || latest === undefined}
      status={status ?? (latest === undefined ? "Connecting to Convex…" : null)}
      error={error}
      onRefresh={refresh}
      modeBadge={latest?.[0]?.demoMode === false ? "FIRECRAWL LIVE" : "DEMO_MODE"}
    />
  );
}
...
function DashboardShell({
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-10 sm:px-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
            Live FX snapshot + short AI tip. Backend on{" "}
            <span className="text-emerald-300">Convex</span>, scrape path via{" "}
            <span className="text-orange-300">Firecrawl</span> (or DEMO fixtures).
          </p>
        </div>
      ...
      {setupBanner && (
        <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 p-4 text-sm text-amber-100">
          ...
          <p className="mt-1 text-amber-100/80">
            Showing local DEMO fixtures so you can click around. For the full reactive
            Convex backend: run <code className="rounded bg-black/30 px-1">npx convex dev</code>,
            copy <code className="rounded bg-black/30 px-1">NEXT_PUBLIC_CONVEX_URL</code> into{" "}
            <code className="rounded bg-black/30 px-1">.env.local</code>, then restart. See{" "}
      ...
      {(status || error) && (
        <p className={`text-sm ${error ? "text-rose-400" : "text-zinc-400"}`}>
          {error ?? status}
        </p>
      )}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {latest.length === 0 && (
            <div className="col-span-full rounded-2xl border border-dashed border-zinc-700 p-8 text-center text-sm text-zinc-500">
              No pulses yet — hit <strong className="text-zinc-300">Refresh rates</strong>.
            </div>
          )}
          {latest.map((p) => (
            <article
            ...
            >
              ...
              <div className="mt-3 flex gap-4 text-xs text-zinc-500">
                {p.bid != null && <span>Bid {formatRate(p.bid)}</span>}
                {p.ask != null && <span>Ask {formatRate(p.ask)}</span>}
              </div>
              <p className="mt-3 truncate text-xs text-zinc-500">{p.sourceLabel}</p>
              <p className="mt-1 text-[11px] text-zinc-600">
                {formatTime(p.capturedAt)} WAT
              </p>
            </article>
          ))}
        </div>
      ...
      <section>
        ...
        <ul className="divide-y divide-zinc-800 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/50">
          {recent.length === 0 && (
            <li className="p-6 text-sm text-zinc-500">History will appear after refresh.</li>
          )}
          {recent.map((p) => (
            <li key={p._id} className="flex flex-col gap-1 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-medium text-zinc-100">
                  {p.pair}{" "}
                  <span className="font-mono text-emerald-400">₦{formatRate(p.rate)}</span>
                </p>
                <p className="text-xs text-zinc-500">{p.sourceLabel}</p>
              </div>
              <p className="text-xs text-zinc-600">{formatTime(p.capturedAt)} WAT</p>
            </li>
          ))}
        </ul>
      </section>
